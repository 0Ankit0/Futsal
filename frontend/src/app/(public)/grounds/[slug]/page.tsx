...
import { useGroundBySlug } from '@/hooks/use-futsal';
import { Card, CardContent } from '@/components/ui/card';
import { Skeleton } from '@/components/ui/skeleton';
import { MapPin, Star, Clock, CheckCircle, AlertCircle, Lock } from 'lucide-react';
import { useGroundReviews, useGroundSlots, type Slot } from '@/hooks/use-futsal';
...
function GroundDetailClient({ slug }: { slug: string }) {
  ...
  const { data: ground, isLoading: groundLoading } = useGroundBySlug(slug);
  const { data: slots, isLoading: slotsLoading } = useGroundSlots(ground?.id ?? 0, selectedDate);
  const { data: reviews } = useGroundReviews(ground?.id ?? 0);
